    ...
    public partial class Form1 : Form
    {
        ...
        void getWeather()
        {
            using (WebClient webClient = new WebClient())
            {
                ...
                labelDesc.Text = weatherInfo.weather[0].description;
        ...
        void getDefaultCityWeather()
        {
            using (WebClient webClient = new WebClient())
            {
                string url = string.Format("https://api.openweathermap.org/data/2.5/weather?q=Kyiv&appid=3467ab89a677299ca993ed1747d06166");
                var json = webClient.DownloadString(url);
                WeatherInfo.root weatherInfo = JsonConvert.DeserializeObject<WeatherInfo.root>(json);
